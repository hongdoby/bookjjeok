...
|---------------------|----------------|-----------|--------------------------|
| Public-A            | 10.0.0.0/22    | AZ-a      | ALB, NAT Gateway, IGW    |
| Public-B            | 10.0.4.0/22    | AZ-b      | ALB, IGW                 |
| Private-A           | 10.0.8.0/22    | AZ-a      | Worker node1 (BE Pod, AI Pod) |
...
- **Worker node1/2**: 멀티 AZ 배치 (인스턴스 타입: **t3.large**)
  - BE Pod: 백엔드 서비스
  - AI Pod: AI 서비스
...
- **Control Plane**: Kubernetes 마스터 노드 (API Server, etcd, Scheduler, Controller Manager) - 인스턴스 타입: **t3.medium**
- **Worker node1/2**: 각 AZ에 분산 배치된 워커 노드 (인스턴스 타입: **t3.medium**, AI 요구사항에 따라 t3.large 확장 가능)
...
2. **VPC3 공유 관리 VPC (장애 격리 공간)**: DB, 모니터링(Grafana), CI/CD를 별도 VPC로 분리. 특히 운영 클러스터(VPC1/VPC2)에 치명적인 다운타임이 발생하더라도, 독립된 VPC3의 통합 모니터링 시스템은 살아남아 끝까지 장애 원인 추적이 가능하도록 **"장애 격리(Fault Domain Separation)"** 패턴을 적용함 (VPC1/VPC2 내부의 Prometheus 메트릭을 VPC3의 단일 Grafana로 통합).
