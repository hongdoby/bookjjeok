...
#========================================
# VPC Peering Connection (VPC2 <-> VPC3)
#========================================
resource "aws_vpc_peering_connection" "vpc2_to_vpc3" {
  ...
  tags = {
    Name = "${var.prefix}-vpc2-to-vpc3"
    Side = "both"
  }
}

resource "aws_vpc_peering_connection_options" "vpc2_to_vpc3" {
...
# VPC2 -> VPC3 (RDS Proxy, Redis)
#========================================
resource "aws_vpc_security_group_ingress_rule" "rds_proxy_from_vpc2" {
  security_group_id = var.vpc3_sg_rds_proxy_id
  description       = "PostgreSQL from VPC2 backend pods"
  from_port         = 5432
  to_port           = 5432
  ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "redis_from_vpc2" {
  security_group_id = var.vpc3_sg_redis_id
  description       = "Redis from VPC2 backend pods"
  from_port         = 6379
  to_port           = 6379
  ip_protocol       = "tcp"
  cidr_ipv4         = var.vpc2_cidr
...
resource "aws_vpc_security_group_ingress_rule" "bastion_from_vpc2" {
  security_group_id = var.vpc3_sg_bastion_id
  description       = "SSH from VPC2 for testing"
  from_port         = 22
  to_port           = 22
  ip_protocol       = "tcp"
  cidr_ipv4         = var.vpc2_cidr
